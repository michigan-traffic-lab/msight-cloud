...
|__ J2735_J3224
|____ J2735_ASN_1_file.asn
|____ J3224_ASN_1_file.asn
|__ pyv2xlib
|____ __init__.py
|____ SDSM.json
|____ SDSM.py
...
```bash
git clone https://github.com/michigan-traffic-lab/PyV2XLib_ASN.git
cd PyV2XLib_ASN
```

4. Download ASN.1 files (optional)
...
Currently, all latest ASN.1 files (J2735_202211, J3224_202208) have been downloaded and stored in ```./J2735_3224```. Therefore, you can skip this step.

5. Compile ASN.1 files (optional)
...
Currently, the ASN.1 files have been compiled. So you can also skip this step.
...
```
'utf-8' codec can't decode byte 0xb5 in position 69258: invalid start byte
```
You can add the invalid byte to the invalid byte list in `remove_invalid_byte.py`, line 5, and run the following command to remove teh garbled characters:
```
python remvoe_invalid_byte.py
```
...
Sure. If you want to do this, first, you will want to download the ASN.1 files you need. Then compile the files by following step 4. Finally, develop the code that can convert the information into correct format so that the compiled files can encode and decode the message. That's why we need ```./pyv2xlib/SDSMEncoder.py``` and ```./pyv2xlib/SDSMDecoder.py``` in this repo and of course, you need to develop your own msg encoder and decoder.
