...
def sdsm_decoder(hex_msg):
    '''
    Decode SDSM message from hex string to dictionary

    Args:
        hex_msg (str): hex string of SDSM message
    ...
    if msg['messageId'] == 41:
        ...
        for i in range(len(sdsm['objects'])):
            ...
            if 'accel4way' in list(sdsm['objects'][i]['detObjCommon'].keys()):
                if 'lat' in list(sdsm['objects'][i]['detObjCommon']['accel4way'].keys()):
                    if sdsm['objects'][i]['detObjCommon']['accel4way']['lat'] == 2001:
                        sdsm['objects'][i]['detObjCommon']['accel4way']['lat'] = 'unavailable'
                    else:
                        sdsm['objects'][i]['detObjCommon']['accel4way']['lat'] /= 100
                if 'long' in list(sdsm['objects'][i]['detObjCommon']['accel4way'].keys()):
                    if sdsm['objects'][i]['detObjCommon']['accel4way']['long'] == 2001:
                        sdsm['objects'][i]['detObjCommon']['accel4way']['long'] = 'unavailable'
                    else:
                        sdsm['objects'][i]['detObjCommon']['accel4way']['long'] /= 100
                if 'vert' in list(sdsm['objects'][i]['detObjCommon']['accel4way'].keys()):
                    if sdsm['objects'][i]['detObjCommon']['accel4way']['vert'] == -127:
                        sdsm['objects'][i]['detObjCommon']['accel4way']['vert'] = 'unavailable'
                    else:
        ...
        print('Message ID is not 41, it is:', msg['messageId'])
        return msg['value'][1]
